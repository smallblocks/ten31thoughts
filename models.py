...
class Note(Base):
    """Personal note for the resurfacing engine. Author-input, free-form."""
    __tablename__ = "notes"

    note_id = Column(String, primary_key=True, default=gen_id)
    title = Column(String, nullable=True)
    body = Column(Text, nullable=False)
    topic = Column(String, nullable=True)  # same vocabulary as ThesisElement.topic
    tags = Column(JSON, default=list)
    source_url = Column(Text, nullable=True)
    archived = Column(Boolean, default=False)

    # FSRS scheduling state — embedded, mirrors py-fsrs Card field names
    ...
    __table_args__ = (
        ...
        Index("idx_note_fsrs_due", "fsrs_due", "archived"),
    )


class ResurfacingEvent(Base):
    """One row per surfaced note, regardless of which trigger fired."""
    ...
    # Trigger-specific context. Exactly one of these is populated per row,
    # determined by `trigger`. See README of resurfacing module for the matrix.
    ...
    # Engagement signals — drive FSRS rating + future filtering
    engaged_at = Column(DateTime, nullable=True)
    dismissed_at = Column(DateTime, nullable=True)
    rating = Column(Integer, nullable=True)  # FSRS rating 1-4

    note = relationship("Note", foreign_keys=[note_id])

    __table_args__ = (
        Index("idx_resurf_note", "note_id"),
        Index("idx_resurf_digest", "digest_date"),
        Index("idx_resurf_trigger", "trigger"),
    )
